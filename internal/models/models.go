...
// NewModels retruns a Models type with a database connection pool
...
func (m *DBModel) GetWidget(id int) (Widget, error) {
	...
	WHERE 
		id =?`, id)
	err := row.Scan(&w.ID, &w.Name, &w.Description, &w.InventoryLevel, &w.Price, &w.Image, &w.IsRecurring, &w.PlanID, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
		return w, err
	}
...
// Status is the type for transactions statuses
...
// Transaction is the model for transactions
...
func (m *DBModel) InsertTransaction(txn Transaction) (int, error) {
	...
	stmt := `
		insert into transactions
			(amount, currency, last_four, expiry_month, expiry_year, bank_return_code, transaction_status_id, 
...
// Customer is the model for user
...
func (m *DBModel) GetAllOrders() ([]*Order, error) {
	...
	select
		...
		t.bank_return_code, c.id, c.first_name, c.last_name
	from
		orders o
...
func (m *DBModel) GetAllUsers() ([]*User, error) {
	...
	for rows.Next() {
		var u User
		err = rows.Scan(&u.ID, &u.LastName, &u.FirstName, &u.Email, &u.CreatedAt, &u.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (m *DBModel) GetOneUser(id int) (User, error) {
	...
	err := row.Scan(&u.ID, &u.LastName, &u.FirstName, &u.Email, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return u, err
	}
